 ...
 * handle on its own thin scrollbar - used by the campfires list and the
 ...
 * position is exactly what made the old page-based CampfireListScreen's
 ...
 * vertical split).
 */
class CampfireScrollPane extends ClickableWidget {
    ...
    CampfireScrollPane(int x, int y, int width, int height, Row emptyRow, Supplier<List<Row>> rowsSupplier) {
    ...
    protected void renderButton(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        scrollOffset = CampfireUi.lerp((float) scrollOffset, (float) MathHelper.clamp(scrollTarget, 0, max), 0.35F);
        ...
        for (int index = 0; index < visible.size(); index++) {
            ...
            if (rowY + rowHeight >= y && rowY <= y + h) {
                ...
                if (hovered) {
                    context.fill(x, Math.max(rowY, y), x + w - reserve, Math.min(rowY + rowHeight, y + h), CampfireUi.ROW_HOVER_BG);
                }
                ...
                if (index < ROW_STAGGER_CAP) {
                    ...
                    rowAlpha = CampfireUi.easeOutCubic(CampfireUi.progress(start, ROW_FADE_MS));
                }
                if (rowAlpha > 0.02F) {
                ...
                if (index < visible.size() - 1) {
                    context.fill(x, rowY + rowHeight - 1, x + w - reserve, rowY + rowHeight, CampfireUi.ROW_DIVIDER);
                }
            }
            rowY += rowHeight;
        }
        ...
        if (max > 0) {
            ...
            CampfireUi.drawScrollbarTrack(context, trackX, y, SCROLLBAR_WIDTH, h);
            ...
            CampfireUi.drawScrollbarThumb(context, trackX, thumbY, SCROLLBAR_WIDTH, thumbHeight, thumbHovered || draggingThumb);
        }
    }
