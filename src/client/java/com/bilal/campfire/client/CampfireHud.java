...
final class CampfireHud {
    ...
    // Wall-clock based like every other animation in CampfireUi - no
    ...
    private static void render(CampfireClient mod, DrawContext context) {
        ...
        int textX = MARGIN + PADDING + CampfireUi.ICON_WIDTH + 5;
        ...
        if (alpha == 255) {
            CampfireUi.drawCampfireIcon(context, MARGIN + PADDING + 1, MARGIN + PADDING + 1);
        }
        context.drawTextWithShadow(tr, status, textX, MARGIN + PADDING + 1,
                (CampfireUi.TITLE_COLOR & 0xFFFFFF) | (alpha << 24));
        if (detail != null) {
            context.drawTextWithShadow(tr, detail, textX, MARGIN + PADDING + 12,
                    (CampfireUi.MUTED_TEXT & 0xFFFFFF) | (alpha << 24));
        }
    }
}
