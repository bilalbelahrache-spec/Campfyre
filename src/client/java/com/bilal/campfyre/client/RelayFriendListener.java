...
public class RelayFriendListener {
    ...
    private void runAcceptLoop() {
        ...
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            if (stopRequested) {
                System.out.println("[Campfyre] Relay listener bind on port " + port + " abandoned - stop() was called mid-retry.");
                return;
            }
            try {
                ...
                serverSocket = socket;
                running = true;
                System.out.println("[Campfyre] Relay listener ready on 127.0.0.1:" + port + " - point Direct Connect here.");
                break;
            } catch (IOException e) {
                lastFailure = e;
                System.out.println("[Campfyre] Relay listener bind attempt " + attempt + "/" + maxAttempts
        ...
        if (!running) {
            System.out.println("[Campfyre] Giving up binding relay listener on port " + port
        ...
        while (running) {
            try {
                ...
                if (running) {
                    System.out.println("[Campfyre] Relay listener accept() failed: " + e.getMessage());
                }
    ...
    private void onAccepted(Socket clientSocket) {
        ...
        System.out.println("[Campfyre] Local Minecraft client connected - opened relay stream " + streamId);
    ...
    public void feedData(String streamId, byte[] payload) {
        ...
        try {
            ...
            System.out.println("[Campfyre] Failed writing relay data to local stream " + streamId + ": " + e.getMessage());
