...
final class HolePuncher {
    ...
    static ReflectedAddress reflect(String coordinatorHostname, int coordinatorPort, int localPort) {
        ...
        System.out.println("[Campfyre] Coordinator reflect unavailable (proxied coordinator, or unreachable) - trying public STUN servers instead.");
        for (String server : STUN_SERVERS) {
            ReflectedAddress viaStun = reflectViaStun(server, localPort);
            if (viaStun != null) return viaStun;
        }
        System.out.println("[Campfyre] Hole-punch reflect (local port " + localPort + ") failed against the coordinator and every public STUN server.");
        return null;
    }
    ...
    private static ReflectedAddress reflectViaCoordinator(String coordinatorHostname, int coordinatorPort, int localPort) {
        try (Socket probe = new Socket()) {
            ...
            System.out.println("[Campfyre] Coordinator reflect (local port " + localPort + ") failed: " + e.getMessage());
            return null;
        }
    }
    ...
    private static ReflectedAddress reflectViaStun(String hostPort, int localPort) {
        ...
        try (DatagramSocket socket = new DatagramSocket(null)) {
            ...
            System.out.println("[Campfyre] STUN reflect via " + hostPort + " (local port " + localPort + ") failed: " + e.getMessage());
            return null;
        }
    }
