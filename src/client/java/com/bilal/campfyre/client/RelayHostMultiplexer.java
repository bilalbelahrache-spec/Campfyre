...
public class RelayHostMultiplexer {
    ...
    // Generous headroom above any real campfyre (the coordinator's own member
     ...
     * CampfyreClient's handleMessage on the "relay_open" case, which runs
    ...
    public void openStream(String streamId, String fromPlayerId) {
        ...
        if (streams.size() >= MAX_CONCURRENT_STREAMS) {
            System.out.println("[Campfyre] Refusing relay stream " + streamId + " - already at the " + MAX_CONCURRENT_STREAMS + "-stream cap.");
    ...
    private void openStreamBlocking(String streamId, String fromPlayerId) {
        ...
        try {
            ...
            System.out.println("[Campfyre] Could not connect relay stream " + streamId
        ...
        if (existing != null) {
            System.out.println("[Campfyre] Relay stream " + streamId + " already open - closing duplicate connection.");
            try {
                socket.close();
            } catch (IOException ignored) {
            }
        ...
        System.out.println("[Campfyre] Opened relay stream " + streamId + " for " + fromPlayerId);
    ...
    public void feedData(String streamId, byte[] payload) {
        ...
        try {
            ...
            System.out.println("[Campfyre] Failed writing relay data to host stream " + streamId + ": " + e.getMessage());
            if (closeLocal(streamId)) {
