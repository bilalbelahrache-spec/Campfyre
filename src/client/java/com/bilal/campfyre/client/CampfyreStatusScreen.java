...
// Opened from the docked title-screen "Campfyre" button when a group is
...
public class CampfyreStatusScreen extends Screen {
    ...
    private CampfyreButton primaryButton;
    private CampfyreButton leaveButton;
    private CampfyreScrollPane roster;
    ...
    // IN_WORLD exists because the Campfyre keybind opens this screen from
    ...
    public CampfyreStatusScreen(CampfyreClient mod, Screen parent) {
    ...
    // Campfyre" beats a generic label), clamped so a maximal 32-char level
    // name can't push the title art off the panel.
    private static String titleFor(CampfyreClient mod) {
        String name = mod.getActiveWorldName();
        if (name == null || name.isBlank()) return "Your Campfyre";
    ...
    protected void init() {
        ...
        copyButton = this.addDrawableChild(new CampfyreButton(centerX + 52, panelTop + 49, 98, 20,
        ...
        primaryButton = this.addDrawableChild(new CampfyreButton(centerX - 150, panelBottom - 50, 194, 20,
                Text.literal("..."), button -> onPrimary(), true));

        leaveButton = this.addDrawableChild(new CampfyreButton(centerX + 52, panelBottom - 50, 98, 20,
        ...
        leaveButton.setTooltip(net.minecraft.client.gui.tooltip.Tooltip.of(
                Text.literal("Forget this Campfyre on this computer. The world and your friends' access stay untouched.")));

        // v2: the roster is a real CampfyreScrollPane now - no more
        ...
        roster = this.addDrawableChild(new CampfyreScrollPane(centerX - 150, rosterTop, 300, rosterBottom - rosterTop,
                null, this::buildRosterRows));

        // The "Campfyres" nav button is always offered, not just once there
        // are already several - it's also the only way to ADD a second one
        // from a single-campfyre state (Light a New Campfyre / Join a
        // Campfyre live on the list screen). Without this a player with
        // exactly one campfyre had no path to ever reach the list at all.
        ...
        // row already used.
        if (mod.isOwner()) {
            this.addDrawableChild(new CampfyreButton(centerX - 102, panelBottom - 26, 65, 20,
                    Text.literal("Campfyres"), button -> this.client.setScreen(
                            parent instanceof CampfyreListScreen ? parent : new CampfyreListScreen(mod, parent))));
            this.addDrawableChild(new CampfyreButton(centerX - 33, panelBottom - 26, 65, 20,
                    Text.literal("Settings"), button -> this.client.setScreen(new CampfyreWorldSettingsScreen(mod, this))));
            this.addDrawableChild(new CampfyreButton(centerX + 36, panelBottom - 26, 65, 20,
                    Text.literal("Close"), button -> this.close()));
        } else {
            this.addDrawableChild(new CampfyreButton(centerX - 102, panelBottom - 26, 100, 20,
                    Text.literal("Campfyres"), button -> this.client.setScreen(
                            parent instanceof CampfyreListScreen ? parent : new CampfyreListScreen(mod, parent))));
            this.addDrawableChild(new CampfyreButton(centerX + 2, panelBottom - 26, 100, 20,
                    Text.literal("Close"), button -> this.close()));
        }
    ...
    private void refreshDynamicWidgets() {
        ...
        CampfyreClient.CoordinatorStatus status = mod.getStatus();
        ...
        } else if (status != CampfyreClient.CoordinatorStatus.CONNECTED) {
        ...
        switch (primaryAction) {
            case RECONNECT -> setPrimary("Reconnect", status == CampfyreClient.CoordinatorStatus.DISCONNECTED);
    ...
    private void onPrimary() {
        switch (primaryAction) {
            ...
            case CREATE_WORLD -> this.client.setScreen(new CampfyreWorldNameScreen(mod, this));
            case JOIN_HOST -> {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (saveVersion > 0) {
            ...
            context.drawTextWithShadow(this.textRenderer, Text.literal(v),
                    panelRight - 6 - this.textRenderer.getWidth(v), panelBottom - 62, CampfyreUi.MUTED_TEXT);
        }
    }
    ...
    private void renderConnectionLine(DrawContext context, int centerX, int y) {
        ...
        switch (status) {
            case CONNECTED -> {
                ...
                dotColor = CampfyreUi.DOT_CONNECTED;
            }
            case CONNECTING -> {
                label = "Connecting to " + mod.getCoordinatorHost() + "...";
                dotColor = CampfyreUi.DOT_CONNECTING;
                pulse = true;
            }
            case DISCONNECTED -> {
                label = "Offline - can't reach " + mod.getCoordinatorHost();
                dotColor = CampfyreUi.DOT_OFFLINE;
            }
            default -> {
                label = "Not connected";
                dotColor = CampfyreUi.DOT_IDLE;
            }
        }
    ...
    private void renderRosterHeader(DrawContext context, int centerX, int top) {
        List<CampfyreClient.GroupMember> members = mod.getMembers();
        String header = "Around the fire" + (members.isEmpty() ? "" : " (" + members.size() + ")");
        context.drawTextWithShadow(this.textRenderer, Text.literal(header), centerX - 150, top, CampfyreUi.TITLE_COLOR);
        CampfyreUi.drawDivider(context, centerX, top + 10, 300);
    }

    // v2: builds the CampfyreScrollPane's row list fresh every tick (see
    // roster.refresh() in tick()) - no MAX_ROSTER_ROWS cap or "+N more"
    // cutoff anymore, a group of any size scrolls in full instead.
    private List<CampfyreScrollPane.Row> buildRosterRows() {
        List<CampfyreScrollPane.Row> rows = new ArrayList<>();
        if (mod.getStatus() != CampfyreClient.CoordinatorStatus.CONNECTED) {
            rows.add(infoRow("Roster unavailable while offline."));
            return rows;
        }
        List<CampfyreClient.GroupMember> members = mod.getMembers();
        List<CampfyreClient.AwayMember> away = mod.getAwayMembers();
        ...
        for (CampfyreClient.AwayMember a : away) {
            rows.add(awayRow(a));
        }
        return rows;
    }

    private CampfyreScrollPane.Row infoRow(String text) {
        return new CampfyreScrollPane.Row() {
            ...
            public void render(DrawContext context, int x, int y, int width, int mouseX, int mouseY, boolean hovered, float delta) {
                context.drawTextWithShadow(textRenderer, Text.literal(text),
                        x + 4, y + (ROSTER_ROW_HEIGHT - textRenderer.fontHeight) / 2, CampfyreUi.MUTED_TEXT);
            }
        };
    }

    private CampfyreScrollPane.Row memberRow(CampfyreClient.GroupMember m, boolean nextUp) {
        return new CampfyreScrollPane.Row() {
            @Override
            public int height() {
                return ROSTER_ROW_HEIGHT;
            ...
            public void render(DrawContext context, int x, int y, int width, int mouseX, int mouseY, boolean hovered, float delta) {
                ...
                CampfyreUi.drawAvatar(context, textRenderer, m.name(), x + 4, avatarY, avatarSize, m.host(), m.host());
                ...
                if (m.modMismatch()) {
                    tag = "mods differ";
                    tagColor = CampfyreUi.ERROR_COLOR;
                } else if (m.host()) {
                    tag = "hosting";
                    tagColor = CampfyreUi.ACCENT;
                } else if (nextUp) {
                    tag = "next up";
                    tagColor = CampfyreUi.MUTED_TEXT;
                } else {
                    tag = null;
                    tagColor = CampfyreUi.MUTED_TEXT;
                }
                ...
                context.drawTextWithShadow(textRenderer, Text.literal(textRenderer.trimToWidth(name, textMaxWidth)), textX,
                        ...
                        m.you() ? CampfyreUi.ACCENT_BRIGHT : CampfyreUi.TEXT_COLOR);
                if (tag != null) {
    ...
    private CampfyreScrollPane.Row awayRow(CampfyreClient.AwayMember a) {
        return new CampfyreScrollPane.Row() {
            @Override
            public int height() {
                return ROSTER_ROW_HEIGHT;
            ...
            public void render(DrawContext context, int x, int y, int width, int mouseX, int mouseY, boolean hovered, float delta) {
                int avatarSize = 14;
                int avatarY = y + (ROSTER_ROW_HEIGHT - avatarSize) / 2;
                CampfyreUi.drawAvatar(context, textRenderer, a.name(), x + 4, avatarY, avatarSize, false, false);
                int textX = x + 4 + avatarSize + 6;
                ...
                context.drawTextWithShadow(textRenderer, Text.literal(textRenderer.trimToWidth(a.name(), textMaxWidth)), textX,
                        y + (ROSTER_ROW_HEIGHT - textRenderer.fontHeight) / 2, CampfyreUi.MUTED_TEXT);
                context.drawTextWithShadow(textRenderer, Text.literal(tag),
                        x + width - 4 - textRenderer.getWidth(tag),
                        y + (ROSTER_ROW_HEIGHT - textRenderer.fontHeight) / 2, CampfyreUi.MUTED_TEXT);
            }
